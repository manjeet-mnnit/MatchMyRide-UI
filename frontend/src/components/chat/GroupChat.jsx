...
export default function GroupChat({ groupId, onBack }) {
  ...
  useEffect(() => {
      ...
      setGroup((prev) => {
        ...
        return {
          ...
          members: prev.members.map((member) => {
            const updatedMember = data.members.find(
              (m) => m.user === member.user._id,
            );
            return updatedMember
              ? { ...member, isReady: updatedMember.isReady }
              : member;
          }),
        };
      });
    ...
    const handleCountdownStarted = (data) => {
      ...
      setCountdownEndTime(new Date(data.endTime).getTime());
    };
    ...
    const handleRideStarted = (data) => {
      ...
      setGroup((prev) => {
        if (!prev) return prev;
        const adminId =
          typeof prev.admin === "string" ? prev.admin : prev.admin?._id;
        return {
          ...prev,
          status: "closed",
          members: prev.members.filter(
            (m) => m.isReady || (m.user && m.user._id === adminId),
          ),
        };
      });

      // Background API fetch to ensure perfectly synced state
      fetchGroupData().catch(console.error);
    };
  ...
  return (
    <div className="flex flex-col h-full bg-background relative">
      ...
      >
        ...
        <TypingIndicator
          typingUsers={typingUsers.filter((n) => n !== currentUser?.fullName)}
        />
